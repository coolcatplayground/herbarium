...
| `npm test` | Vitest, 517 tests, ~0.5s |
...
| `npm run plates` | re-index public/plates/ after adding an illustration |
| `npm run plates:brief` | regenerate curation/PLATES.md, the drawing list |
...
Drop a file in `public/plates/` named for the specimen slug — `bulbasaur.webp`,
`venusaur-mega.png`, `wo-chien.webp` — and run `npm run plates` (or just
`npm run build`, which does it). Where a plate exists the gallery, the habitat
rooms and the specimen sheet show it; where one does not, the sprite stands in.
Nothing else to wire.

**The subject is the botanical part, not the creature.** A drawing of Bulbasaur
is a drawing of Bulbasaur; a drawing of the bulb is a drawing of a corm, which
is what its field note already says it is. That makes the picture the evidence
for the claim rather than decoration beside it — and it is why this removes the
IP exposure rather than merely relocating it.

`curation/PLATES.md` is the drawing list, generated by `npm run plates:brief`
from the `plant_analogue` of every block. It groups by organ, because that is
where the work collapses: an evolution line usually shares one organ across
stages, so a line is one subject drawn twice or three times rather than three
drawings. 147 specimens, 12 organ groups, and 24 whose analogue names no organ
at all — decide those before drawing, not during.

The manifest `src/data/plates.json` is generated. Never hand-edit it, and note
that it exists to stop the app requesting plates that do not exist: without it
every undrawn specimen costs a failed request on every gallery scroll.
